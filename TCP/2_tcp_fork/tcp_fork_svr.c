...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if(fpid < 0)//fork 失败
            {
                fprintf(stdout, "fork fail,my process id is %d\n", getpid());
            }
            else if(fpid == 0)//子进程返回0
            {
                fprintf(stdout, "fork success,child process id is %d\n", getpid());
                ...
                fprintf(stdout, "send reply msg, %d byte\n", ret);
                close(connect_fd);//子进程关闭
                fprintf(stdout, "exit child process , id is %d\n", getpid());
                exit(0);//kill child process
            }
            else//父进程返回子进程的PID
            {
                fprintf(stdout, "fork success,father process id is %d, fpid %d\n", getpid(), fpid);
                close(connect_fd);//父进程关闭
            }
...
close
fork后需关闭两次，fork后子进程继承了父进程的这个fd
从进程的角度说，子进程关闭子进程的fd，父进程关闭父进程的fd，互不影响！

僵尸进程处理Z+
wait->waitpid(sigaction)函数
singal函数注册信号处理回调函数
#endif